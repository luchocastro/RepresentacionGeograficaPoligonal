...
//                split.SortTheChunks(Path.Combine(Path.GetDirectoryName(ColumnXY.PathFields ), ColumnXY.Name, "Temp"), ColumnXY.ColumnType()  );
 ...
 ;
        ...
        public async Task<HexFileDTO> UploadFileAsync(Stream fileStream, string contentType, string ID)
        {
            var fileCount = 0;
            long totalSizeInBytes = 0;

            var boundary = contentType;// GetBoundary(MediaTypeHeaderValue.Parse(contentType));
