...
namespace Hexagon.Cluster
{
   ...
   public  class Point
    ...
    {
        public  static List<List<Point>> Get (List<Model.Point> Points, double eps, int minPts)
        {
            var points = Points.Select(x => new Point(x.X, x.Y)).ToList();
            return DBSCAN.GetClusters(points, eps, minPts);
             }
